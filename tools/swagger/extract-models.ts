...
export function extractModels(jsonData: any, config: Config, enums: DTO_File[]): DTO_File[] {
  ...
  const filteredModels = filterModels(models, config)

  const modelFilesList: DTO_File[] = filteredModels.map(model => {
    let propertiesList = '';
...
function filterModels(enumsArr: Model[], config: Config): Model[] {
  const { models } = config
  const { title } = models
